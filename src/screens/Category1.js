  ...
  return (
    <View style={styles.productItem}>
      ...
      <View>
        ...
        <Text style={styles.categoryName}>{data.genre}</Text>
        <View>
...
const Category1 = ({ navigation, route }) => {
    ...
    const filtered = medicineData.filter((item) => {
      if (searchQuery.length > 0) {
        ...
        const genreMatch = item.genre
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
        return symptomsMatch || nameMatch || genreMatch;
      } else {
        return true;
      }
    });
      ...
      <View style={{ marginTop: 20 }}>
        <View style={styles.heading}>
          ...
          <View style={styles.categories}>
            <Text style={{ fontSize: 30 }}>{route.params.category}</Text>
          </View>
        </View>
        {medicineData.length > 0 ? (
          <View>
            ...
            <FlatList
              data={filteredData}
              renderItem={(item) => (
                <Item data={item.item} navigation={navigation} />
              )}
          ...
          <View style={styles.emptyText}>
            <Text>No medicine in this category</Text>
          </View>
        )}
      </View>
    </View>
